...
const CustomEditor = dynamic(
  () => {
    return import("../../../components/Dashboard/form/custom-editor-Update_Journalist.tsx");
  },
  { ssr: false }
);
...
const upadteArticles: React.FC<InferGetServerSidePropsType<typeof getServerSideProps>> = ({ repo, repo2 }) => {
  ...
  const breadcrumbs = [
    ...
    <Link underline="hover" key="3" color="inherit" href={`/journalist/articles/${router.query.id}`}>
      Detail
    </Link>,
  ];
  return (
    <>
      ...
      <Box sx={{ m: 5 }} />
      <CustomEditor bigData={repo} repo={repo2} />
    </>
  );
};
